...
function createInvoice(): any {
 return request('POST', SERVER_URL + '/invoice', {
   json: {
...
describe('GET /invoice — listInvoices', () => {
 ...
 describe('Successful cases', () => {
   test('returns 200 with valid parameters (no filters)', () => {
     const res = request('GET', SERVER_URL + '/invoice', {
       headers,
       timeout: TIMEOUT_MS,
     });
   ...
   test('returns 200 with valid from_date and to_date', () => {
     ...
     const res = request('GET', SERVER_URL + '/invoice?from_date=2024-01-01&to_date=2026-12-31', {
       headers,
       timeout: TIMEOUT_MS,
     });
