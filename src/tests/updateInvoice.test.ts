...
describe('PUT /v1/invoice/:invoice_id', () => {
  describe('error cases', () => {
    test('NOT_FOUND - invoice not found', () => {
      ...
      const res = requestUpdateInvoice(invoice.body.invoice_id + 9999, { buyer_name: 'New Test Buyer' });
    ...
    test('INVALID_REQUEST - buyer name contains invalid characters', () => {
      const invoice = requestCreateInvoice('Test Buyer', '12345678901', 'Test Supplier', '98765432101', new Date('2025-01-01'), new Date('2025-02-01'),
        [{ item_name: 'item', quantity: 2, unit_price: 50.0, unit_code: 'ea', total_price: 100.0 }], 0.1, [{ bank_name: 'ANZ', account_number: '123456789', bsb_abn_number: '012-345', payment_method: 'bank_transfer' }]) as HttpReturnObject<{ invoice_id: string; status: string; created_at: string }>;
      expect(invoice.statusCode).toStrictEqual(201);

      const res = requestUpdateInvoice(invoice.body.invoice_id, { buyer_name: 'New Test Buyer!!!!' });
    ...
    test('INVALID_REQUEST - buyer ABN is <11 digits long', () => {
      ...
      const res = requestUpdateInvoice(invoice.body.invoice_id, { buyer_abn: '1234567890' });
      expect(res.statusCode).toStrictEqual(400);
      expect(res.body).toStrictEqual({
        error: 'INVALID_REQUEST',
        message: expect.any(String)
    ...
    test('INVALID_REQUEST - buyer ABN is >11 digits long', () => {
      ...
      const res = requestUpdateInvoice(invoice.body.invoice_id, { buyer_abn: '12345678901233' });
      expect(res.statusCode).toStrictEqual(400);
      expect(res.body).toStrictEqual({
        error: 'INVALID_REQUEST',
        message: expect.any(String)
    ...
    test('INVALID_REQUEST - supplier name contains invalid characters', () => {
      ...
      const res = requestUpdateInvoice(invoice.body.invoice_id, { supplier_name: 'New Test Supplier :PP' });
      expect(res.statusCode).toStrictEqual(400);
      expect(res.body).toStrictEqual({
        error: 'INVALID_REQUEST',
        message: expect.any(String)
    ...
    test('INVALID_REQUEST - supplier ABN is <11 digits long', () => {
      ...
      const res = requestUpdateInvoice(invoice.body.invoice_id, { supplier_abn: '9876543210' });
      expect(res.statusCode).toStrictEqual(400);
      expect(res.body).toStrictEqual({
        error: 'INVALID_REQUEST',
        message: expect.any(String)
    ...
    test('INVALID_REQUEST - supplier ABN is >11 digits long', () => {
      ...
      const res = requestUpdateInvoice(invoice.body.invoice_id, { supplier_abn: '9876543210111' });
      expect(res.statusCode).toStrictEqual(400);
      expect(res.body).toStrictEqual({
        error: 'INVALID_REQUEST',
        message: expect.any(String)
    ...
    test('INVALID_REQUEST - issue date is of an invalid format', () => {
      ...
      const res = requestUpdateInvoice(invoice.body.invoice_id, { issue_date: new Date('not-a-date') });
      expect(res.statusCode).toStrictEqual(400);
      expect(res.body).toStrictEqual({
        error: 'INVALID_REQUEST',
        message: expect.any(String)
    ...
    test('INVALID_REQUEST - issue date is after payment date', () => {
      ...
      const res = requestUpdateInvoice(invoice.body.invoice_id, { issue_date: new Date('2025-02-03') });
      expect(res.statusCode).toStrictEqual(400);
      expect(res.body).toStrictEqual({
        error: 'INVALID_REQUEST',
        message: expect.any(String)
    ...
    test('INVALID_REQUEST - payment date is of an invalid format', () => {
      ...
      const res = requestUpdateInvoice(invoice.body.invoice_id, { payment_date: new Date('not-a-date') });
      expect(res.statusCode).toStrictEqual(400);
      expect(res.body).toStrictEqual({
        error: 'INVALID_REQUEST',
        message: expect.any(String)
    ...
    test('INVALID_REQUEST - payment date is before issue date', () => {
      ...
      const res = requestUpdateInvoice(invoice.body.invoice_id, { payment_date: new Date('2024-01-01') });
      expect(res.statusCode).toStrictEqual(400);
      expect(res.body).toStrictEqual({
        error: 'INVALID_REQUEST',
        message: expect.any(String)
    ...
    test('INVALID_REQUEST - new item details are invalid (name)', () => {
      ...
      const res = requestUpdateInvoice(invoice.body.invoice_id, { item_details: [{ item_name: 'updated item!!!!! hehe' }] });
      expect(res.statusCode).toStrictEqual(400);
      expect(res.body).toStrictEqual({
        error: 'INVALID_REQUEST',
        message: expect.any(String)
    ...
    test('INVALID_REQUEST - new item details are invalid (quantity)', () => {
      ...
      const res = requestUpdateInvoice(invoice.body.invoice_id, { item_details: [{ quantity: -5 }] });
      expect(res.statusCode).toStrictEqual(400);
      expect(res.body).toStrictEqual({
        error: 'INVALID_REQUEST',
        message: expect.any(String)
    ...
    test('INVALID_REQUEST - new item details are of an invalid format (quantity)', () => {
      ...
      const res = requestUpdateInvoice(invoice.body.invoice_id, { item_details: [{ quantity: 'two' }] });
      expect(res.statusCode).toStrictEqual(400);
      expect(res.body).toStrictEqual({
        error: 'INVALID_REQUEST',
        message: expect.any(String)
    ...
    test('INVALID_REQUEST - new item details are invalid (unit_price)', () => {
      ...
      const res = requestUpdateInvoice(invoice.body.invoice_id, { item_details: [{ unit_price: 59 }] });
      expect(res.statusCode).toStrictEqual(400);
      expect(res.body).toStrictEqual({
        error: 'INVALID_REQUEST',
        message: expect.any(String)
    ...
    test('INVALID_REQUEST - new item details are of an invalid format (unit_price)', () => {
      ...
      const res = requestUpdateInvoice(invoice.body.invoice_id, { item_details: [{ unit_price: 'fifty' }] });
      expect(res.statusCode).toStrictEqual(400);
      expect(res.body).toStrictEqual({
        error: 'INVALID_REQUEST',
        message: expect.any(String)
    ...
    test('INVALID_REQUEST - new item details are of an invalid format (unit_price)', () => {
      ...
      const res1 = requestUpdateInvoice(invoice.body.invoice_id, { item_details: [{ unit_code: 'ea!!/!!' }] });
      ...
      const res2 = requestUpdateInvoice(invoice.body.invoice_id, { item_details: [{ unit_code: 30 }] });
    ...
    test('INVALID_REQUEST - new item details are invalid (total_price)', () => {
      ...
      const res = requestUpdateInvoice(invoice.body.invoice_id, { item_details: [{ unit_price: 150 }] });
      expect(res.statusCode).toStrictEqual(400);
      expect(res.body).toStrictEqual({
        error: 'INVALID_REQUEST',
        message: expect.any(String)
    ...
    test('INVALID_REQUEST - new item details are of an invalid format (total_price)', () => {
      ...
      const res = requestUpdateInvoice(invoice.body.invoice_id, { item_details: [{ unit_price: 'one-hundred' }] });
      expect(res.statusCode).toStrictEqual(400);
      expect(res.body).toStrictEqual({
        error: 'INVALID_REQUEST',
        message: expect.any(String)
    ...
    test('INVALID_REQUEST - tax rate is of an invalid format', () => {
      ...
      const res = requestUpdateInvoice(invoice.body.invoice_id, { tax_rate: '10 percent' });
      expect(res.statusCode).toStrictEqual(400);
      expect(res.body).toStrictEqual({
        error: 'INVALID_REQUEST',
        message: expect.any(String)
    ...
    test('INVALID_REQUEST - invalid bank_name', () => {
      ...
      const res = requestUpdateInvoice(invoice.body.invoice_id, { payment_details: [{ bank_name: 'fake-bank' }] });
      expect(res.statusCode).toStrictEqual(400);
      expect(res.body).toStrictEqual({
        error: 'INVALID_REQUEST',
        message: expect.any(String)
    ...
    test('INVALID_REQUEST - invalid payment_method', () => {
      ...
      const res = requestUpdateInvoice(invoice.body.invoice_id, { payment_details: [{ payment_method: 'scam' }] });
      expect(res.statusCode).toStrictEqual(400);
      expect(res.body).toStrictEqual({
        error: 'INVALID_REQUEST',
        message: expect.any(String)
  ...
  describe('success cases', () => {
    test('successfully updates all invoice fields', () => {
      ...
      const beforeUpdate = requestGetInvoice(invoice.body.invoice_id) as HttpReturnObject<Invoice>;
